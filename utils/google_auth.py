...
# Get redirect URI from environment variable or use a default value
REDIRECT_URI = os.environ.get("OAUTH_REDIRECT_URI", "https://ai-chat-studio.dartopia.uk/")
...
def create_oauth_flow():
    ...
    st.session_state["debug_redirect_uri"] = redirect_uri
...
def get_authorization_url():
    ...
    if not st.session_state.get("debug_redirect_shown", False):
        with st.expander("Debug Info (Click to expand)", expanded=False):
            ...
            st.write(f"Using redirect URI: {flow.redirect_uri}")
...
def validate_oauth_callback(callback_url, state):
    ...
    # Check if state validation should be bypassed (for development only)
    bypass_state_check = os.environ.get("BYPASS_STATE_CHECK", "").lower() == "true"
    ...
    if bypass_state_check:
        ...
        # Normal validation - only reached if BYPASS_STATE_CHECK is not true
    ...
    try:
        ...
        with st.expander("Debug OAuth Callback", expanded=False):
            ...
            st.write(f"Debug - Configured redirect URI: {flow.redirect_uri}")
